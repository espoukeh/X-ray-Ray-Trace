...
int main(int argc, char *argv[])
{
	...
	int useZero = 0;
		...
		{"zero",              0, NULL,               'z'},
         ...
         /* Short options */
        while ((c = getopt_long(argc, argv, "hi:o:b:c:f:p:s:z",
                longopts, NULL)) != -1)
        {
	...
	if ( inFileName != NULL ) {
        ...
        /* loop over vectors */
        while (!feof(in)) {
                double absorption_prob, r, d, distance, *d_rand;
                Vector x, pointToCenter;
		Ray myRay;
                c = read_ray(in, &myRay);
                if (c != 0) {
                        if (feof(in)) break;
		...
		Normal = unit(sub(myRay.pos,Center)); // make positive N is a unit normal vector
		...
		//calculate where myRay.pos is with respect to sphere center and if on sphere
		//x = add( myRay.pos, mult(myRay.v,distance));
		x = mult(myRay.v,dot(sub(Center, myRay.pos), myRay.v));
		pointToCenter = sub(x, sub(Center, myRay.pos));
		if (magnitude(pointToCenter) <= R) {
			//on sphere
			r = sqrt( pow(R,2) - dot(pointToCenter, pointToCenter));
			myRay.pos = add(myRay.pos,mult(myRay.v, (magnitude(x) - r)));
                	myRay.l = myRay.l + (magnitude(x)-r);
        	//      incidentAngle =  acos(dot(mult(myRay.v,-1),Normal));
       		//      outAngle = asin((n1*sin(incidentAngle))/n2);
               		d = 2*r;
				
               		absorption_prob = 1 - exp(- absorb_factor*d);
			d_rand =  rand_double(R_DOUBLE,1);
			if (d_rand[0] < absorption_prob) {
                    	  	myRay.i = 0.0; // Absorb the ray 
                	} 
               // 	else {	
		//	myRay.i = myRay.i * absorption_prob;
			myRay.pos = add(myRay.pos,mult(myRay.v,d));
			myRay.l = myRay.l + d;
		}
		//print ray
		write_ray(out, myRay);
